...
import getFloorElementConfig from "../../../../lib/get-floor-element-config";
...
export default class ObjectsOptions extends React.Component {
	...
	folderClicked(folder) {
		floorElementsByFolder[folder].isOpen = !floorElementsByFolder[folder].isOpen;
		this.forceUpdate();
	}
	/*
	 *	@function onMouseUp
	...
	onMouseDown(feClicked, event){
		...
		let maxLayer = DiagramStore.getMinFloorElementLayer();
		...
		ToolActions.updateActiveTool("selector");
		ToolActions.updateMetadata(Immutable.Map({
			dragX: 0,
			dragY: 0,
			isDragging: true,
	 ...
	 */
	render() {
		let folderTypes = Object.keys(floorElementsByFolder);
		let chevronFolders = [];
		let filteredFolders = {};
		let headerContent = [];
		...
		if (this.state.search.isSearching && this.state.search.searchInput.length) {
			...
			folderTypes.forEach((folder) => {
				floorElementsByFolder[folder].floorElements.forEach((fe) => {
					if (fe.name.toLowerCase().includes(this.state.search.searchInput.toLowerCase())) {
						if (!filteredFolders.folder) {
							...
							filteredFolders.folder.floorElements.push(fe);
						}
					}
				});
			});
			...
			filteredFolderKeys.forEach((folder) => {
				let key = "object-options-folder-" + uuid.v4();

				chevronFolders.push(
					<div key={key} className="st-vm-objects-options-folder"
						onClick={this.folderClicked.bind(this, folder)}>

						<label className="st-vm-objects-options-folder-type">{folder}</label>
					</div>
				);

				let floorElementsLIs = [];
				filteredFolders[folder].floorElements.forEach((fe) => {
			...
			folderTypes.forEach((folder) => {
				...
				if (!floorElementsByFolder[folder].isOpen) {
					chevronFolders.push(
						<div key={key} className="st-vm-objects-options-folder"
							onClick={this.folderClicked.bind(this, folder)}>
